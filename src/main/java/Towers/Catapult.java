...
public class Catapult extends Tower{
    ...
    * */
    public Catapult(int x, int y, int r1, int damage, int r2, int ir, mapObject[][] map) {
    ...
    public void inflictDamage(mapObject[][] map) {
        ...
        int index = (this.x*ARENA_SIZE) + this.y; //ir refers to a stone's fallout radius, index initialized to this coz can't hit
        for (int dy = -this.r2; dy <= this.r2; ++dy)
            for (int dx = -(int) (Math.sqrt(r2 * r2 - dy * dy)); dx <= (int) (Math.sqrt(r2 * r2 - dy * dy)); ++dx)
                if ((dx*dx + dy*dy > this.r1*this.r1) && this.x + dx >= 0 && this.x + dx < ARENA_SIZE && this.y + dy >= 0 && this.y+dy < ARENA_SIZE && map[this.x + dx][this.y + dy].monster != null)
                    index = updateDistribution(this.x + dx, this.y + dy, this.x, this.y, this.r1, this.r2, this.ir, index, map, distribution);
        if (index != (this.x * ARENA_SIZE) + this.y) {
            int killX = index / ARENA_SIZE, killY = index % ARENA_SIZE;
            for (int killDy = -this.ir; killDy <= this.ir; ++killDy)
                for (int killDx = -(int) (Math.sqrt(this.ir * this.ir - killDy * killDy)); killDx <= (int) (Math.sqrt(this.ir * this.ir - killDy * killDy)); ++killDx)
...
//                        System.out.println("Catapult inflicted: (" + (killX + killDx) + ", " + (killY + killDy) + " )");
