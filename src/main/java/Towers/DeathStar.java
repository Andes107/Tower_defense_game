...
public class DeathStar extends Tower{
    public int fallout;
    public DeathStar(int x, int y, int r1, int damage, int fallout, mapObject[][] map) {
    ...
    public void inflictDamage(mapObject[][] map) {
        ...
        if (x1 == this.x) {
            for (int i = x1 - this.fallout; i <= x1 + this.fallout; ++i)
                for (int j = Math.min(this.y, y1) - this.fallout; j <= Math.max(this.y, y1); ++j)
                    if (i >= 0 && i < ARENA_SIZE && j >= 0 && j < ARENA_SIZE && map[i][j].monster != null)
                        map[i][j].monster.health -= this.damage;
        } else if (y1 == this.y) {
            for (int u = y1 - this.fallout; u <= y1 + this.fallout; ++u)
            ...
            for (int a = Math.min(this.x, x1); a <= Math.max(this.x, x1); ++a) {
                for (int b = (int) (numerator * a / denumerator + c) - this.fallout; b <= (int) (numerator * a / denumerator + c) + fallout; ++b)
